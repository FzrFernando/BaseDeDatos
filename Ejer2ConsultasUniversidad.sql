...
SELECT DISTINCT a.NOMBRE, a.COSTEBASICO 
FROM ASIGNATURA a
...
WHERE a.IDPROFESOR = p.IDPROFESOR AND p.DNI = p2.DNI AND p2.CIUDAD LIKE 'Sevilla';
...
AND aa.IDASIGNATURA = a2.IDASIGNATURA AND a2.NOMBRE LIKE 'Seguridad Vial';
...
WHERE a.IDALUMNO = aa.IDALUMNO AND aa.IDASIGNATURA = a2.IDASIGNATURA 
...
WHERE p.DNI = p2.DNI AND p2.IDPROFESOR = a.IDPROFESOR AND a.IDASIGNATURA = aa.IDASIGNATURA 
AND aa.IDALUMNO = a2.IDALUMNO AND p.NOMBRE LIKE 'Jorge' AND p.APELLIDO LIKE 'Saenz';
...
WHERE p.DNI = p2.DNI AND p2.IDPROFESOR = a.IDPROFESOR AND a.IDASIGNATURA = aa.IDASIGNATURA 
AND aa.IDALUMNO = a2.IDALUMNO AND a2.DNI = p3.DNI AND p.NOMBRE LIKE 'Jorge' AND p.APELLIDO LIKE 'Saenz';
...
AND a2.DNI = p.DNI AND a.CREDITOS > 4.5;
...
SELECT a.NOMBRE, a.CREDITOS *10 AS numero_horas, a2.DNI 
