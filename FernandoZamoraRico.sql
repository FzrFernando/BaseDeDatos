...
SELECT DISTINCT p.CIP, p.FECHA_ESTRENO, p.RECAUDACION
FROM PROYECCION p 
...
SELECT s.CINE, SUM(NVL(s.AFORO,0)) AS AFORO_TOTAL
FROM SALA s 
GROUP BY s.CINE
...
SELECT DISTINCT c.*, s.SALA, s.AFORO, p2.TITULO_P 
FROM CINE c, SALA s, PROYECCION p, PELICULA p2
WHERE c.CINE = s.CINE AND s.SALA = p.SALA 
AND p.CIP = p2.CIP
...
SELECT DISTINCT t.TAREA, COUNT(t.TAREA) AS NUMERO_PERSONAS
FROM TRABAJO t
GROUP BY t.TAREA;
...
SELECT c.CINE, c.CIUDAD_CINE, COUNT(DISTINCT p.CIP) AS NUM_PELICULAS_PROYECTADAS
...
WHERE p.CIP = p2.CIP AND p2.SALA = s.SALA 
AND s.CINE = c.CINE AND UPPER(p.NACIONALIDAD) LIKE 'EE.UU'
...
SELECT p.TITULO_P, SUM(NVL(p2.RECAUDACION,0)) AS RECAUDACION_TOTAL
FROM PELICULA p, PROYECCION p2
WHERE p.CIP = p2.CIP 
...
SELECT MAX(NVL(p.PRESUPUESTO,0)) AS PRESUPUESTO_MAXIMO, MIN(NVL(p.PRESUPUESTO,0)) AS PRESUPUESTO_MINIMO
FROM PELICULA p;

--17. Explica en qué consiste el OUTER JOIN e indica un ejemplo justificándolo e
--incluyendo la sentencia correspondiente.
/*Es una unión de tablas que nos muestra los datos que cumplen con la condición
 * incluyendo a los valores nulos.
 * 
 * SELECT e.CODEMP, e.PROFESION, d.CODDEPT
 * FROM e.EMP, d.DEPT
...
SELECT COUNT(t.TAREA), p.TITULO_P 
